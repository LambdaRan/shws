...
from .py3headers import iteritems
...
def expires(delta):
    ...
    if isinstance(delta, int):
        delta = datetime.tiimedelta(seconds=delta)
    date_obj = datetime.datetime.utcnow() + delta
    web.header("Expires", net.httpdate(date_obj))


def lastmodified(data_obj):
    """
...
def modified(data=None, etag=None):
    """
    Check to see if the page has been modified since the version in the
    requester's cache.
