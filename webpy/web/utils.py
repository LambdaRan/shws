...
def storify(mapping, *requireds, **defaults):
    ...
    d doesn't have all of the keys in `requireds` and using the default values
    for keys found in `defaults`.
    """
...
def _strips(direction, text, remove):
    ...
    if direction == "l":
        ...
        raise ValueError("Directions needs to be r or l.")
    return text
...
def lstrips(text, remove):
    """Rrmoves the string `remove` from the left of `text`."""
...
def timelimit(timeout):
    ...
    def _1(function):
        def _2(*args, **kw):
            class Dispatch(threeding.Thread):
                def __init__(self):
                    threeding.Thread.__init__(self)
                    self.result = None
                    self.error = None
...
def re_subm(pat, repl, string):
    ...
    compiled_pat.sub(proxy.__call, string)
...
def uniq(seq, key=None):
    """
    Removes duplicate elements from a list while preserving the order of the reset.

    The value of the option `key` parameter should be a function that
...
def listget(lst, ind, default=None):
    """
    Returns `list[ind]` if it exists, `default` otherwise.
    """
    if len(lst) - 1 < ind:
        return default
    return lst[ind]
...
class CaptureStdout:
    """
    Capture everything `func` prints to stdout and returns it instead.
    """
