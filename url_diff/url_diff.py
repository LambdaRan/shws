...
# author    : lambda RDG
# Date&Time : 2019/03/24 13:05:16

# WebSite: ttps://github.com/google/url_diff/blob/master/url_diff.py
"""
...
# TODO(macpd): investigate making this namedtuple
class ParamDiffEntry(object):
    ...
    def _valid_diff_type(self, diff_type):
        if (diff_type != self.LEFT_ONLY  and 
            diff_type != self.RIGHT_ONLY and
...
class UrlDiffer(object):
    ...
    UNIX_SLASH = "/"
    URL_ESCAPE_CHAR = "%"
    URL_ESCAPE_SEQ_LEN = 3 # excepted lenght URL espace sequence, aka len("%25")
    ...
    def _get_hostname(self, url):
        """Parses the hostname form a URL"
        
        Find hostname between scheme and fist unix slash.
        """
    ...
    def _diff_hostname(self, left, right):
    ...
    def _get_params(self, url):
        ...
        if self.PATH_DELIM not in url:
            return param_dict
        param_pos = url.find(self.PATH_DELIM) + 1
        for token in url[param_pos:].split(self.PARAM_DELIM):
            if not token:
                continue
            if "=" not in token:
                token_key = token
    ...
    def _diff_param(self, left_params, right_params):
        """Returns a list of the diffence between dicts on key/values.

        First all keys that exit in both URLs are compared, that keys only in the
        left, followed by keys only in the right.

        Args:
            left_patam: dict; param name -> values dict of the left URL.
            right_param: dict; param name -> values dict of the right URL.
        Returns:
            List of ParamDiffEntry of differneces between the left and right params.
        """
        diffs = []
        ...
        for common_key in key_intersection:
            ...
            if left_diff and right_diff:
                ...
                # if no diff skip to net iteration
                continue
            diffs.append(
                ParamDiffEntry(
                    common_key, 
        ...
        for right_key in right_key_diff:
            diffs.append(ParamDiffEntry(
                right_key, None, right_key_diff[right_key], ParamDiffEntry.RIGHT_ONLY
            ))
        return diffs
    ...
    def _do_diff(self):
        ...
        if self._wants_hostname_diff:
            ...
            self._diff_hostname(self._left_hostname, self._right_hostname)
        ...
        if not self._left_params_dict == self._right_params_dict:
            self._diffs.extend(self._diff_param(self._left_params_dict, self._right_params_dict))
    ...
    def left_params(self):
        """Returns a deep copy of the left params dict."""
        return copy.deepcopy(self._left_params_dict)
    
    def right_params(self):
        """Returns a deep copy of the right parames dict."""
...
def main():
    """Params args, inits and prints differ, and exit with appropriate value."""
    # TODO(macpd): provide option for second diff dilimeter. This would allow one
